                            ...
                            <div class="col-sm-8 pb-0 ">
                                <form id="cartForm" name="cartForm" class="form-horizontal">
                                    @foreach($orders as $key1 => $order)
                                        ...
                                        <input type="hidden" name="product_name[]" value="{{$order->product->productInfor[0]->name}}">
                                    @endforeach
                                    <div class="cart-wrapper sm-touch-scroll">
                                        <table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
                                            ...
                                            @foreach($orders as $key => $order)
                                                <tr class="woocommerce-cart-form__cart-item cart_item" id="order-{{ $key }}">
                                                    ...
                                                    <td class="product-thumbnail col-sm-1">
                                                        <div class="flickity-viewport" style="height: 489.359px; touch-action: pan-y;">
                                                            <div class="flickity-slider" style="left: 0px; transform: translateX(0%);">
                                                                @foreach($order->product->getMedia('product') as $key => $image)
                                                                    @if($key == 0)
                                                    ...
                                                    <td class="product-name" data-title="Sản phẩm">
                                                        <a href="https://www.noithatgiakhanh.com/san-pham/bo-giuong-ngu-tan-co-dien-mbk311bg/">{{ $order->product->productInfor[0]->name }}</a>
                                                        <div class="show-for-small mobile-product-price">
                                                            <span class="mobile-product-price__qty">1 x </span>
                                                            <span class="woocommerce-Price-amount amount" id="price">&nbsp{{ $order->product->price }}<span class="woocommerce-Price-currencySymbol">$</span></span>
                                                        </div>
                                                    </td>

                                                    <td class="product-price" data-title="Giá">
                                                        <input type="hidden" id="order-price-{{ $key }}" value="{{ $order->product->price }}">
                                                        <span class="woocommerce-Price-amount amount" >{{ $order->product->price }}&nbsp;<span class="woocommerce-Price-currencySymbol">$</span></span>
                                                    </td>
                                                    <td class="product-quantity" data-title="Số lượng">
                                                        <div class="quantity buttons_added">
                                                            <label class="screen-reader-text" for="quantity">Số lượng</label>
                                                            <input type="button" value="-" class="minus button is-form" id="minus-qty" data_key="{{ $key }}">
                                                            <input type="text" id="quantity-{{ $key }}" class="input-text qty text" name="qty" value="{{ $order->qty }}" title="SL" size="4" inputmode="numeric">
                                                            <input type="button" value="+" class="plus button is-form" id="plus-qty" data_key="{{ $key }}">
                                                        </div>
                                                    </td>

                                                    <td class="product-subtotal" data-title="Tổng">
                                                        <input type="hidden" name="total_money" id="total_money" value="{{ $order->product->price * $order->qty }}">
                                                        <span class="woocommerce-Price-amount amount" id="total-money-{{ $key }}">{{ $order->product->price * $order->qty }}&nbsp;<span class="woocommerce-Price-currencySymbol">$</span></span>
                                                    </td>
                                                </tr>
                                            @endforeach
                                            <tr>
                                                <td colspan="6" class="actions clear">
                                                    ...
                                                    <div class="float-right">
                                                        <button type="submit" class="btn btn-danger" id="addCart">Cập nhật giỏ hàng</button>
                                                    </div>
                                                </td>
                                            </tr>
                                            </tbody>
                                    ...
                                    <div class="cart_totals calculated_shipping">
                                        ...
                                        <table cellspacing="0" class="shop_table shop_table_responsive">
                                            ...
                                            <tr class="cart-subtotal">
                                                <th>Tạm tính</th>
                                                <td data-title="Tạm tính"><span class="woocommerce-Price-amount amount" id="total-cost-one">{{ $total_money }}&nbsp;<span class="woocommerce-Price-currencySymbol">$</span></span>
                                                </td>
                                            </tr>
                                            ...
                                            <tr class="order-total">
                                                ...
                                                <td data-title="Tổng">
                                                    <strong>
                                                <span class="woocommerce-Price-amount amount" id="total-cost">{{ $total_money}}&nbsp;<span class="woocommerce-Price-currencySymbol">$</span>
                                                </span>
                                                    </strong>
                                                </td>
                                            </tr>
                                        ...
                                        <div class="wc-proceed-to-checkout">
                                            ...
                                            <a href="https://www.noithatgiakhanh.com/checkout/" class="checkout-button button alt wc-forward">
...
@section('page-scripts')
    <script>
        ...
        $(document).on('click', '#plus-qty', function (event) {
            ...
            var id_price = '#order-price-' + data_key;
            let price = parseInt($(id_price).val());
            var quantity = '#quantity-' + data_key;
            var value = parseInt($(quantity).val());
            console.log(price);
            $(quantity).val(value + 1);
            let vl_new = parseInt($(quantity).val());
            var total_money = '#total-money-' + data_key;
            $(total_money).text(vl_new * parseInt(price) + '$');
